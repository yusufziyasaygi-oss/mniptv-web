(() => {
  ...
  if (location.hostname === 'mavinokta.pro' && location.protocol === 'https:') {
  ...
  function publicSource(source) {
    return source ? { type:source.type, name:source.name || 'IPTV', key:source.key, server:'Bağlı IPTV kaynağı' } : null;
  }
  ...
  function normalizeProviderServer(raw) {
    ...
    let u; try { u = new URL(value); } catch { throw appError('Sunucu adresi geçersiz.', 400, 'invalid_server'); }
    if (u.hostname.toLowerCase() !== PROVIDER_HOSTNAME) {
      throw appError('Bu web sürümü bağlı IPTV kaynağı için hazırlanmıştır. Uygulamada kullandığın sunucu adresini aynen gir.', 400, 'provider_mismatch');
    }
    // Sağlayıcının API cevabında bildirdiği çalışan HTTP portu 2086'dır.
    // Girilen sağlayıcı adresindeki eski port bilgisi dikkate alınmaz; doğrulanmış 2086 portu kullanılır.
    return `http://${PROVIDER_HOST}/`;
  }
  ...
  function providerApiUrl(source, action = '', params = {}) {
    ...
    q.set('username', source.username);
    q.set('password', source.password);
    if (action) q.set('action', action);
    for (const [k,v] of Object.entries(params)) q.set(k, String(v));
    return `http://${PROVIDER_HOST}/player_api.php?${q}`;
  }
  ...
  async function providerFetch(url, kind = 'json') {
    ...
    try {
      ...
      throw appError('IPTV sunucusuna ulaşılamadı.', 502, 'upstream_unreachable');
    }
    if (!response.ok) {
  ...
  async function browserSession(method, payload = null) {
    ...
    if (type === 'xtream') {
      ...
      const source = {...temp,key:stableId(`${PROVIDER_HOSTNAME}|${username}`),epgUrl:`http://${PROVIDER_HOST}/xmltv.php?username=${encodeURIComponent(username)}&password=${encodeURIComponent(password)}`};
  ...
  function routeInfo() {
    const map = {
      ...
      continue:['İzlemeye Devam Et','Kaldığın yerden devam et'],favorites:['Favoriler','Kaydettiğin içerikler'],downloads:['İndirilenler','İnternet olmadan izlemeye hazır'],search:['Arama','Tüm içeriklerde ara'],settings:['Ayarlar','MN IPTV tercihleri']
  ...
  function renderSettings(){
    return `<div class="settings-grid">
      ...
      <section class="settings-card"><h3>Kaynak</h3><p>${esc(state.source?.name||'MN IPTV')} · Bağlı IPTV kaynağı</p><button id="logoutButton" class="danger-button">Kaynağı Kapat</button></section>
  ...
  function mediaUrl(item){
    ...
    let ext=String(item.ext||(item.section==='live'?'m3u8':'mp4')).replace(/[^a-zA-Z0-9]/g,'');
    if(item.section==='live')ext='m3u8';
    return `http://${PROVIDER_HOST}/${route}/${encodeURIComponent(source.username)}/${encodeURIComponent(source.password)}/${encodeURIComponent(item.providerId)}.${ext||'mp4'}`;
  }
  ...
  async function startPlayback(item,opts={}){
    ...
    const url=mediaUrl(item);const isHls=String(item.ext||'').toLowerCase()==='m3u8'||item.section==='live';
    const v=playerEls.video;v.pause();v.removeAttribute('src');v.load();destroyHls();
